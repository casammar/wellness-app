...
**Test framework**: this repo uses Node's built-in test runner.
...
- Import pattern:
  ...
  ```
- Follow the patterns in `test/helpers.test.js` for structure and style.
- If the change is to server routes rather than pure functions, test the
  helper logic that the route delegates to — do not spin up the server.
